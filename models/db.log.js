...
export class login{
  ...
  static async addAccount(nickname, email, password, token){ // Creates a [USER]'s account after Signup
    const request = await sql`
      INSERT INTO Users VALUES((SELECT MAX(userid) FROM Users)+1, ${nickname}, ${email}, ${password}, 1, ${token});
    `
  }
